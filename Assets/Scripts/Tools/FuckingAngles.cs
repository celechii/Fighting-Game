...
public static class FuckingAngles {
	...
	public static Vector2 ClampDirectionAngle(Vector2 dir, Vector2 middleDir, float variance, float dirOffset, float middleOffset) {
		return ClampDirectionAngle(dir, AngleFromDir(middleDir, -variance + middleOffset), AngleFromDir(middleDir, variance + middleOffset), dirOffset);
	}

	public static Vector2 ClampDirectionAngle(Vector2 dir, Vector2 minDir, Vector2 maxDir, float dirOffset) {
		return ClampDirectionAngle(dir, AngleFromDir(minDir), AngleFromDir(maxDir), dirOffset);
	}

	public static Vector2 ClampDirectionAngle(Vector2 dir, float min, float max, float dirOffset) {
		return DirFromAngle(ClampAngle(AngleFromDir(dir, dirOffset), min, max));
	}

	public static float ClampAngle(float angle, float min, float max) {
		float signedMin = AddToSignedAngle(min, -angle);
		float signedMax = AddToSignedAngle(max, -angle);

		if (signedMin <= 0 && signedMax >= 0)
			return angle;

		if (Mathf.Abs(signedMin) < Mathf.Abs(signedMax))
			return min;
		return max;
	}
	...
	public static Quaternion QuaternionFromDir(Vector2 direction, float offset) {
		return QuaternionFromAngle(AngleFromDir(direction, offset));
	}

	public static Quaternion QuaternionFromDir(Vector2 direction) {
		return QuaternionFromAngle(AngleFromDir(direction));
	}

	public static Quaternion QuaternionFromAngle(float angle, float offset) {
		return Quaternion.Euler(0, 0, AddToAngle(angle, offset));
	}

	public static Quaternion QuaternionFromAngle(float angle) {
		return Quaternion.Euler(0, 0, Angle(angle));
	}
}